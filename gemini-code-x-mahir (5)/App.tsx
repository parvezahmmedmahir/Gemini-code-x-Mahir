...
const App: React.FC = () => {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden font-sans bg-black text-white selection:bg-blue-500/30">
      {showLogin && (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#1e1e1e] border border-white/10 p-8 rounded-3xl max-w-sm w-full text-center shadow-2xl">
             <div className="w-12 h-12 bg-white rounded-xl mx-auto flex items-center justify-center mb-6 shadow-glow">
                <span className="font-bold text-black text-xl">M</span>
             </div>
             <h2 className="text-2xl font-bold text-white mb-2">Welcome Back</h2>
             <p className="text-slate-400 text-sm mb-8">Sign in to sync your history.</p>
             <button onClick={handleLogin} disabled={isLoggingIn} className="w-full bg-white text-black font-bold py-3 rounded-full hover:bg-gray-200 transition-colors flex items-center justify-center gap-3">
               {isLoggingIn ? <Loader2 className="animate-spin" /> : <>Continue with Google</>}
             </button>
             <button onClick={() => setShowLogin(false)} className="mt-4 text-xs text-slate-500 hover:text-white">Cancel</button>
          </div>
        </div>
      )}
      ...
      <main className="flex-1 h-full relative flex flex-col bg-[#171717]">
